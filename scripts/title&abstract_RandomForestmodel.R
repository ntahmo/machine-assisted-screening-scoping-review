...
boot_mat <- replicate(B, {
  ...
  c(
    ...
    prec = as.numeric(cm_b$byClass["Pos Pred Value"]),
    f1   = as.numeric(cm_b$byClass["F1"])
  )
})
...
ci_prec <- quantile(boot_mat["prec",], probs = c(0.025, 0.975), na.rm = TRUE)
ci_prec <- quantile(boot_mat["prec",], probs = c(0.025, 0.975), na.rm = TRUE)
ci_f1   <- quantile(boot_mat["f1",],   probs = c(0.025, 0.975), na.rm = TRUE)
